...
def zabbix_service_remove(hostname):
    try:
        group = zapi.host.get(output=['itemid', 'name'], filter={'host': hostname})
        # print(group)
        for host in group:
            a=zapi.hostgroup.massremove(groupids="216", hostids=host['hostid'])
            print('Хост выведен из режима "Обслуживание"')
            print(a)
    except Exception:
...
if hostname == 'www.eurodate.com test':
    zabbix_service_remove(hostname)
else:
    print('Данный скрипт запрещено использовать на\
          хостах кроме "www.eurodate.com test"')
